...
public class CarBehaviour : MonoBehaviour {
	...
	public void SetWheelShape(GameObject WheelSha){
		Debug.Log (WheelSha.name);
		wheelShape = WheelSha;
		Debug.Log (wheelShape.name);
		for (int i = 0; i < wheels.Length; ++i) 
		{
			var wheel = wheels [i];
		...
		/*Destroy (wheelShape);
		wheelShape = Instantiate (ws, new Vector3 (1, -2, 1), Quaternion.identity) as GameObject;*/
		//wheelShape = GameObject.Find(ws.name);
	}
